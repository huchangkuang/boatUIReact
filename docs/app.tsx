...
export const App: FC = () => {
  const [select, setSelect] = useState<Menu>("quickStart");
    ...
    <Layout style={{ width: "100vw", height: "100vh" }}>
      <Header className={scm("header")}>
        <Layout className={scm("header-layout")}>
          ...
          <Content className={scm("header-content")}></Content>
        </Layout>
      </Header>
      <Layout style={{ height: "calc(100vh - 64px)" }}>
        <Aside className={scm("aside")}>
          {menuList.map((i) => (
            <div key={i.title} className={scm("aside-menuGroup")}>
              {i.title && (
                <div className={scm("aside-menuGroup-title")}>{i.title}</div>
              )}
              {i.list.map((j) => (
                <div
                  key={j.key}
                  className={cs(
                    scm("aside-menuItem"),
                    select === j.key ? scm("aside-menuItem__selected") : "",
                  )}
                  onClick={() => {
                    setSelect(j.key);
